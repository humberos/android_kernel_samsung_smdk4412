...
ssize_t show_UV_uV_table(struct cpufreq_policy *policy, char *buf) 
{
	...
	if (buf) {
		for (i = exynos_info->max_support_idx; i<=exynos_info->min_support_idx; i++) {
			if (exynos_info->freq_table[i].frequency==CPUFREQ_ENTRY_INVALID) 
				continue;
...
void acpuclk_set_vdd(unsigned int khz, unsigned int vdd)
{
	...
	for (i = exynos_info->max_support_idx; i<=exynos_info->min_support_idx; i++) {
		...
		//always round down
		if (new_vdd % 12500) 
			new_vdd = (new_vdd / 12500) * 12500;

		exynos_info->volt_table[i] = new_vdd;
	}
}

ssize_t store_UV_uV_table(struct cpufreq_policy *policy, const char *buf, size_t count) 
{
	int i = 0;
	int j = 0;
